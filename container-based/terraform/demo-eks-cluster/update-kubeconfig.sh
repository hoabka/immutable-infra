...
SPIN_MANAGED=true
NAMESPACE=us-west-2
CONTEXT=demo-eks-cluster

### update kubeconfig
function kube_config () {
  aws eks update-kubeconfig --name demo-eks-cluster --profile svmc --no-verify-ssl
...
EOF
  ...
  kubectl config set-credentials demo-eks-cluster-sa --token $TOKEN
  kubectl config set-context $CONTEXT --cluster=arn:aws:eks:us-west-2:633834615594:cluster/demo-eks-cluster --user=demo-eks-cluster-sa --namespace=$NAMESPACE
